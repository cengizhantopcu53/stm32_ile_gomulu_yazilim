...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint32_t led_data[2] = {GPIO_PIN_8 , (uint32_t)(~GPIO_PIN_8)};
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  HAL_DMA_Start(&hdma_memtomem_dma1_channel1, (uint32_t)&led_data[1], (uint32_t)&GPIOC->BSRR, 1);
	  HAL_DMA_PollForTransfer(&hdma_memtomem_dma1_channel1, HAL_DMA_FULL_TRANSFER, HAL_MAX_DELAY);
	  HAL_Delay(500);

	  HAL_DMA_Start(&hdma_memtomem_dma1_channel1, (uint32_t)&led_data[0], (uint32_t)&GPIOC->BSRR, 1);
	  HAL_DMA_PollForTransfer(&hdma_memtomem_dma1_channel1, HAL_DMA_FULL_TRANSFER, HAL_MAX_DELAY);
	  HAL_Delay(500);
  }
  /* USER CODE END 3 */
