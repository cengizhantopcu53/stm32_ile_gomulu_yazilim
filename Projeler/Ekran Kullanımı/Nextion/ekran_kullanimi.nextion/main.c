...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      if(UART_available())
      {
    	  HAL_UART_Receive(&huart1, (uint8_t *)rx_buff, 8, HAL_MAX_DELAY);

          if(rx_buff[1]==0x65 && rx_buff[2]==0x00 && rx_buff[3]==0x01 && rx_buff[4]==0x01 && rx_buff[5]==0xFF && rx_buff[6]==0xFF && rx_buff[7]==0xFF){
